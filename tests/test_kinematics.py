...
def test_cartesian_offset_window_accepts_first_and_last_keyframes(
    tmp_path,
) -> None:
    model_path = tmp_path / "six.xml"
    model_path.write_text(SIX_DOF_MODEL, encoding="utf-8")
    ...
    trajectory = TrajectoryData(
        np.arange(41) / 100,
        {name: np.zeros(41) for name in names},
        names,
    )
    engine = KinematicsEngine(
        adapter,
    ...
    offset = np.asarray([0.01, 0.0, 0.0])

    from_first, first_report, first, last = engine.cartesian_offset_window(
        trajectory, "tool", 0.0, 0.0, 0.2, offset, np.eye(3), 0.1
    )
    assert not first_report.failed_frames
    assert (first, last) == (0, 30)
    first_positions, _ = engine.sample_body_poses(
        from_first, "tool", np.asarray([0, 20, 30])
    )
    np.testing.assert_allclose(
        first_positions[:, 0], [0.01, 0.01, 0.0], atol=5e-5
    )

    to_last, last_report, first, last = engine.cartesian_offset_window(
        trajectory, "tool", 0.2, 0.4, 0.4, offset, np.eye(3), 0.1
    )
    assert not last_report.failed_frames
    assert (first, last) == (20, 40)
    last_positions, _ = engine.sample_body_poses(
        to_last, "tool", np.asarray([20, 40])
    )
    np.testing.assert_allclose(
        last_positions[:, 0], [0.0, 0.01], atol=5e-5
    )
