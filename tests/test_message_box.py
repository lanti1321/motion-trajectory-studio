...
def test_message_icon_does_not_consume_the_text_width(monkeypatch) -> None:  # type: ignore[no-untyped-def]
    application = QApplication.instance() or QApplication([])
    captured = []
    monkeypatch.setattr(
        ReadableMessageBox,
    ...
    QMessageBox.critical(None, "错误", "错误正文")
    icon = next(
        label
        for label in captured[0].findChildren(QLabel)
        if label.objectName() == "qt_msgboxex_icon_label"
        or (
            label.pixmap() is not None
            and not label.pixmap().isNull()
        )
    )
    assert icon.maximumWidth() <= 36
    text_labels = [
        label
        for label in captured[0].findChildren(QLabel)
        if label is not icon and label.text()
    ]
    assert text_labels
    assert max(label.minimumWidth() for label in text_labels) >= 440


def test_five_line_error_uses_scrollable_diagnostics(monkeypatch) -> None:  # type: ignore[no-untyped-def]
    application = QApplication.instance() or QApplication([])
    captured = []
    monkeypatch.setattr(
        ReadableMessageBox,
    ...
    message = "\n".join(f"关节 q{index} 连续性错误" for index in range(6))
    QMessageBox.critical(None, "编辑失败", message)
    editors = captured[0].findChildren(QPlainTextEdit)
    assert len(editors) == 1
    assert editors[0].toPlainText() == message
