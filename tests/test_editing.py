...
def test_splice_inserts_overlapping_channels_and_holds_the_rest() -> None:
    times = np.arange(101) / 10.0
    base = TrajectoryData(
        times,
        {"left": np.sin(times), "right": np.cos(times)},
        ["left", "right"],
    )
    insert_times = np.arange(21) / 10.0
    inserted = TrajectoryData(
        insert_times,
        {"left": np.full(21, 2.0), "extra": insert_times.copy()},
        ["left"],
    )
    output = splice_at(base, inserted, at=3.0, transition_duration=0.0)
    first = base.nearest_frame(3.0)
    count = inserted.frame_count
    np.testing.assert_allclose(
        output.channels["left"][first + 1 : first + 1 + count],
        np.full(count, 2.0),
    )
    np.testing.assert_allclose(
        output.channels["right"][first + 1 : first + 1 + count],
        np.full(count, base.channels["right"][first]),
    )
    assert "extra" not in output.channels


def test_splice_rejects_trajectories_with_no_overlapping_channels() -> None:
    times = np.arange(11) / 10.0
    base = TrajectoryData(times, {"left": times.copy()}, ["left"])
    other = TrajectoryData(times, {"right": times.copy()}, ["right"])
    with pytest.raises(ValueError, match="没有可重叠"):
        splice_at(base, other, at=0.3)
