...
def remap_labels(
    ...
    if kind in {"speed", "speed_ramp"}:
        ...
        return result
    if kind in {"splice_at", "insert_module"} and (
        kind == "splice_at"
        or str(operation.get("category") or "trajectory_clip")
        in {"trajectory_clip", "skill"}
    ):
...
def remap_time(
    ...
    if kind in {"splice_at", "insert_module"}:
        return float(operation.get("at") or operation.get("start") or 0.0)
