...
def validate_operation_continuity(
    ...
    selected = channels
    if kind in {"splice_at", "insert_module"}:
        start = float(operation.get("at") or operation.get("start") or 0.0)
        new_span = float(operation.get("_actual_inserted_span", 0.0))
        if new_span <= 0 and "start" in operation and "end" in operation:
            new_span = float(operation["end"]) - float(operation["start"])
    ...
    check_c2 = (
        ...
        and kind not in {"splice_at", "insert_module", "sample_patch", "quintic_keyframe", "move_segment"}
    )
    ...
    for boundary_time in boundary_times:
        ...
        for name, values in metrics.items():
            ...
            if abs(values["velocity_jump"]) > 0.25 * velocity_scale:
                velocity_limit = 0.25 * velocity_scale
                report.issues.append(
                    ValidationIssue(
                        "error",
                        "c1_discontinuity",
                        f"{name} 在 {boundary_time:.6g}s（帧 {frame}）存在 C1 速度不连续："
                        f"接缝速度差 {abs(values['velocity_jump']):.4g}，"
                        f"局部允许值 {velocity_limit:.4g}。"
                        "片段移动保持原波形，不会自动修改接缝速度；"
                        "请移动到速度方向更接近的位置，或对接缝做短时平滑。",
                        name,
                        frame,
                    )
                )
            if abs(values["acceleration_jump"]) > 0.75 * acceleration_scale:
                acceleration_limit = 0.75 * acceleration_scale
                report.issues.append(
                    ValidationIssue(
                        "error",
                        "c2_discontinuity",
                        f"{name} 在 {boundary_time:.6g}s（帧 {frame}）存在 C2 加速度不连续："
                        f"接缝加速度差 {abs(values['acceleration_jump']):.4g}，"
                        f"局部允许值 {acceleration_limit:.4g}。"
                        "请扩大平滑范围或调整接缝位置。",
                        name,
                        frame,
                    )
