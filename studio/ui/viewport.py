...
from PySide6.QtGui import QImage, QKeyEvent, QMouseEvent, QPixmap, QShowEvent, QWheelEvent
...
_RENDER_SIZE_SLOP = 8
_VIEW_SIZE_HINT = QSize(640, 360)
_VIEW_MIN_SIZE = QSize(240, 135)
_MAX_PREVIEW_SHADOWSIZE = 2048
_MAX_PREVIEW_GEOMS = 20000
...
def qimage_from_rgb(rgb: np.ndarray) -> QImage:
    """Build a QImage whose row stride is 4-byte aligned.

    Qt's RGB888 decoder misreads rows when ``3 * width`` is not a multiple of
    4, which shows up as a scrambled / striped viewport.
    """
    frame = rgb_frame_for_qt(rgb)
    height, width, _ = frame.shape
    row_bytes = width * 3
    aligned_stride = (row_bytes + 3) & ~3
    if int(frame.strides[0]) == aligned_stride:
        buffer = frame
        data = buffer.data
        bytes_per_line = int(buffer.strides[0])
    else:
        buffer = np.zeros((height, aligned_stride), dtype=np.uint8)
        buffer[:, :row_bytes] = frame.reshape(height, row_bytes)
        data = buffer.data
        bytes_per_line = aligned_stride
    image = QImage(
        data, width, height, bytes_per_line, QImage.Format.Format_RGB888
    )
    return image.copy()


def prepare_model_for_preview(model: mujoco.MjModel, width: int, height: int) -> None:
    """Shrink editor-only GPU buffers so dense meshes do not scramble the view."""
    ...
    if int(model.vis.quality.shadowsize) > _MAX_PREVIEW_SHADOWSIZE:
        model.vis.quality.shadowsize = _MAX_PREVIEW_SHADOWSIZE
...
class MuJoCoViewport(QLabel):
    ...
    def __init__(self, parent=None) -> None:  # type: ignore[no-untyped-def]
        ...
        self.setMinimumSize(_VIEW_MIN_SIZE)
    ...
    def refresh(self) -> None:
        ...
        try:
            ...
            image = qimage_from_rgb(self.renderer.render())
    ...
    def sizeHint(self) -> QSize:  # type: ignore[override]
        return QSize(_VIEW_SIZE_HINT)

    def minimumSizeHint(self) -> QSize:  # type: ignore[override]
        return QSize(_VIEW_MIN_SIZE)

    def showEvent(self, event: QShowEvent) -> None:  # noqa: N802
        super().showEvent(event)
        if self.adapter is not None and self.renderer is None:
            try:
                self._ensure_renderer(force=True)
                self.refresh()
            except Exception as error:
    ...
    def _desired_render_size(self) -> tuple[int, int]:
        ...
        width = int(round(max(4, self.width()) * pixel_ratio))
        height = int(round(max(4, self.height()) * pixel_ratio))
        width -= width % 4
        height -= height % 4
        return max(4, min(width, 3840)), max(4, min(height, 2160))
    ...
    def _ensure_renderer(self, force: bool = False) -> None:
        ...
        if (
            ...
            and abs(self._render_size[0] - width) <= _RENDER_SIZE_SLOP
            and abs(self._render_size[1] - height) <= _RENDER_SIZE_SLOP
        ):
            return
        self._close_renderer()
        prepare_model_for_preview(self.adapter.model, width, height)
        self.renderer = mujoco.Renderer(
            self.adapter.model,
            height=height,
            width=width,
            max_geom=_MAX_PREVIEW_GEOMS,
        )
        self._render_size = (width, height)

    def _resize_renderer(self) -> None:
        if self.adapter is None or not self.isVisible():
            return
        try:
            self._ensure_renderer()
            self.refresh()
