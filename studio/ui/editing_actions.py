...
class BlankTrajectoryDialog(QDialog):
    def __init__(self, parent=None) -> None:  # type: ignore[no-untyped-def]
        ...
        self.hz.setRange(1.0, 1000.0)
        self.hz.setDecimals(3)
        self.hz.setValue(50.0)
        form.addRow("时长 (s)", self.duration)
        form.addRow("采样率 (Hz)", self.hz)
...
class EditingActionsMixin:
    ...
    def _install_copy_paste_shortcuts(self) -> None:
        application = QApplication.instance()
        if application is not None:
            application.installEventFilter(self)

    def _remove_copy_paste_shortcuts(self) -> None:
        application = QApplication.instance()
        if application is not None:
    ...
    def eventFilter(self, watched, event):  # type: ignore[no-untyped-def]
        if event.type() in (QEvent.Type.ShortcutOverride, QEvent.Type.KeyPress):
            if self._should_handle_copy_paste_shortcut(event):
    ...
    def _should_handle_copy_paste_shortcut(self, event) -> bool:  # type: ignore[no-untyped-def]
        ...
        while widget is not None:
            if isinstance(
                widget, (QLineEdit, QPlainTextEdit, QTextEdit, QAbstractSpinBox)
            ):
                return False
    ...
    def add_copy(self) -> None:
        if getattr(self, "_handling_copy_paste", False):
            return
        self._handling_copy_paste = True
        try:
            ...
            self._trajectory_clipboard = (
                id(data), selected[0], selected[1], channels
            )
            self.statusBar().showMessage(
            ...
            self._handling_copy_paste = False
    ...
    def add_splice(self) -> None:
        ...
        try:
            ...
            return
        shared = [name for name in data.channels if name in inserted.channels]
        if not shared:
            QMessageBox.critical(
                self, "轨迹拼接", "拼接轨迹没有可重叠的通道"
            )
            return
        ...
        channels = [name for name in channels if name in inserted.channels] or shared
        recommended = recommend_splice_transition_duration(
            data, inserted, insert_at, channels
        )
        overlap_note = ""
        if set(inserted.channels) != set(data.channels):
            overlap_note = (
                "\n将插入重叠通道；当前轨迹中未重叠的通道保持原姿态。"
            )
        duration, ok = QInputDialog.getDouble(
            self,
            "轨迹拼接",
            f"过渡段时长(s)\n推荐最低时长约 {recommended:.3f}s{overlap_note}",
            recommended,
            recommended,
            60.0,
            3,
    ...
    def new_blank_trajectory(self) -> None:
        if self.source is not None and not self._confirm_discard_or_save():
            return
        values = self._blank_channel_values()
        if not values:
            QMessageBox.information(
                self, "新建空白轨迹", "请先加载模型，或先导入一条轨迹以获得电机通道"
            )
            return
        dialog = BlankTrajectoryDialog(self)
        if dialog.exec() != QDialog.DialogCode.Accepted:
            return
        trajectory = blank_trajectory(
            values, duration=float(dialog.duration.value()), hz=float(dialog.hz.value())
        )
        self._adopt_generated_trajectory(trajectory, "blank")

    def generate_trajectory_from_playhead(self) -> None:
        data = self._rendered()
        if data is None:
            self.new_blank_trajectory()
            return
        channels = self._selected_motor_channels(data) or list(data.position_channels)
        if not channels:
            QMessageBox.information(self, "轨迹生成器", "没有可用于关键点的电机通道")
            return
        time = float(data.times[self.frame])
        targets = {name: float(data.channels[name][self.frame]) for name in channels}
        start = max(0.0, time - 0.15)
        end = min(data.duration, time + 0.15)
        if end <= start:
            QMessageBox.information(self, "轨迹生成器", "当前轨迹太短，无法写入关键点")
            return
        operation = {
        ...
        if self._add_operation(operation):
            self.statusBar().showMessage(f"已在 {time:.3f}s 写入生成器关键点")

    def _blank_channel_values(self) -> dict[str, float]:
        if self.adapter is not None:
            joints = independent_joint_positions(self.adapter)
            mapping = self.project.joint_mapping
            if mapping:
                values: dict[str, float] = {}
                for channel, joint in mapping.items():
                    if joint in joints:
                        values[str(channel)] = float(joints[joint])
                if values:
                    return values
            if joints:
        ...
        if data is None:
            return {}
        frame = min(self.frame, data.frame_count - 1)
        return {
            name: float(data.channels[name][frame])
            for name in data.position_channels
        }

    def _adopt_generated_trajectory(
        self, trajectory: TrajectoryData, name: str
    ) -> None:
        export_root = Path(tempfile.gettempdir()) / "motion_studio_generated"
        export_root.mkdir(parents=True, exist_ok=True)
        path = export_trajectory(
            trajectory, export_root / f"{name}_{uuid.uuid4().hex[:8]}.npz"
        )
        self._import_trajectory_path(path, preserve_project=True, confirm=False)
        if self.timeline_engine is None:
            self.timeline_engine = TimelineEngine(
                trajectory, validator=self._operation_validator
            )
        self.statusBar().showMessage(f"已创建空白轨迹：{path.name}")
