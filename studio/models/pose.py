...
def joint_pose_patch_operation(
    ...
    if selected_range is not None:
        first_time, last_time = sorted(
            (float(selected_range[0]), float(selected_range[1]))
        )
        first = data.nearest_frame(first_time)
        last = data.nearest_frame(last_time)
        if last < first:
        ...
        return {
            ...
            "channel_values": {
                channel: np.full(
                    last - first + 1, float(channel_value)
                ).tolist()
            },
        }
