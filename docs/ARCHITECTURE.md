...
English-only presentation normalization lives in `studio/ui/i18n.py`. It also
translates legacy internal diagnostics at the UI boundary. Global font selection lives in `style.py`; all user
...
The section below is retained only as historical internal documentation; the
shipped application and supported documentation language are English.
